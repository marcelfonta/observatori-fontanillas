import { fetchLocalityForecast, fetchNearbyStations, searchMunicipalities } from '../services/weather-api.js';
...
async function selectPlace(location,{updateUrl=true}={}){
  input.value=location.name;suggestions.hidden=true;result.hidden=false;result.innerHTML='<div class="panel municipality-loading"><strong>Preparant la consulta…</strong><span>Separem la previsió del model de les observacions reals.</span></div>';
  ...
  try{
    const [forecast,stations]=await Promise.allSettled([fetchLocalityForecast(location),fetchNearbyStations('now',location)]);
    if(forecast.status==='rejected')throw forecast.reason;
    result.innerHTML=renderForecast(forecast.value.location,forecast.value.weather)+(stations.status==='fulfilled'?renderStations(stations.value,location):renderStations({stations:[]},location));
    status.textContent=`Resultats actualitzats per a ${placeLabel(location)}.`;
  }catch(error){result.innerHTML='<div class="panel municipality-loading is-error"><strong>No hem pogut carregar aquest municipi.</strong><span>Comprova la connexió o prova una altra localitat.</span></div>';status.textContent='Consulta no disponible temporalment.';}
}
