...
export function analyticsServiceState({googleMeasurementId='',cloudflareBeacon=false}={}){if(cloudflareBeacon)return {label:'Cloudflare actiu',className:'is-ok',provider:'cloudflare'};if(googleMeasurementId)return {label:'Google actiu',className:'is-ok',provider:'google'};return {label:'No configurat',className:'is-muted',provider:'none'};}
...
function renderIntegrations(integrations={}){const analytics=analyticsServiceState({googleMeasurementId:CONFIG.analyticsMeasurementId,cloudflareBeacon:cloudflareAnalyticsDetected()});const states={weatherUnderground:serviceState(Boolean(integrations.weatherUnderground)),database:serviceState(Boolean(integrations.database)),contact:serviceState(Boolean(integrations.contact)),pushClient:serviceState(Boolean(CONFIG.oneSignalAppId)),pushWorker:serviceState(Boolean(integrations.push)),admin:serviceState(Boolean(integrations.admin)),analytics,social:{label:'Preparat',className:'is-muted'}};document.querySelectorAll('#admin-integrations > [data-integration]').forEach(row=>{const state=states[row.dataset.integration]||serviceState(false);const badge=row.querySelector('b');badge.textContent=state.label;badge.className=state.className;});const required=Boolean(integrations.weatherUnderground&&integrations.database&&integrations.admin);setPill('admin-integrations-pill',required?'Principals actives':'Configuració incompleta',required?'is-ok':'is-warning');return analytics;}
...
async function renderDashboard(payload,requestLatency){
  ...
  latestDiagnostic={...payload,client:{webVersion:'19.1.1',requestLatencyMs:requestLatency,pwa:await localPwaStatus(),publication:await renderPublicationReadiness(),performance:renderPerformanceSnapshot(),analyticsConfigured:analytics.provider!=='none',analyticsProvider:analytics.provider,oneSignalClientConfigured:Boolean(CONFIG.oneSignalAppId),socialAutomation:'prepared-not-connected'},incidents:[...incidents]};
