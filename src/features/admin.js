...
import { publicationState } from '../core/publication-state.js';
...
function renderOperations(operations={},schedule={}){
  ...
  text('admin-social-schedule',`${schedule.social||'06:45'} · ${schedule.timeZone||'Europe/Madrid'}`);
  ...
  text('admin-youtube-schedule',`${schedule.youtube||'06:05,19:45'} · ${schedule.timeZone||'Europe/Madrid'}`);
  text('admin-youtube-operation',youtube?.checkedAt?`${publicationState(youtube).label} · ${formatDate(youtube.checkedAt)}`:'Pendent del primer disparador');
  text('admin-meta-video-operation',metaVideo?.checkedAt?`${metaVideo.status==='healthy'?'Completada':metaVideo.status==='degraded'?'Processant':'Error'} · ${formatDate(metaVideo.checkedAt)}`:schedule.metaVideoEnabled?`Activada · ${schedule.metaVideo||'06:45,20:30'}`:'Automatització desactivada');
  ...
  text('admin-buffer-operation',bufferTikTok?.checkedAt?`${publicationState(bufferTikTok).label} · ${formatDate(bufferTikTok.checkedAt)}`:'Encara no comprovat');
  text('admin-buffer-x-operation',bufferX?.checkedAt?`${publicationState(bufferX).label} · ${formatDate(bufferX.checkedAt)}`:'Encara no comprovat');
...
async function runSocialStoryTest(slot){
  const button=element(slot==='morning'?'admin-social-stories-morning':'admin-social-stories-evening');if(!button)return;
  const label=slot==='morning'?'matí':'vespre';
  if(!window.confirm(`Vols publicar ara el Short del ${label} com a Story independent a Instagram i Facebook? És una prova real i no compartirà la targeta clicable del Reel.`))return;
  button.disabled=true;button.textContent='Publicant Stories…';socialFeedback(`Enviant la prova de Stories del ${label} a Instagram i Facebook…`,'warning');
  try{const payload=await adminApi('/admin/social-stories/test',{method:'POST',body:{slot}});const accepted=(payload.outcomes||[]).filter(item=>item.ok).map(item=>item.channel);const failed=(payload.outcomes||[]).filter(item=>!item.ok);if(failed.length)throw new Error(`${payload.error||'La prova ha quedat incompleta'} ${accepted.length?`Canals acceptats: ${accepted.join(' i ')}.`:''}`.trim());socialFeedback(`Prova completada: ${accepted.join(' i ')} han acceptat la Story. Ara comprova visualment les dues publicacions.`, 'ok');await fetchStatus({skipDrafts:true});}
  catch(error){socialFeedback(`La prova de Stories no s’ha completat: ${error.message}`,'error');recordIncident('Prova de Stories',error.message);}
  finally{button.disabled=false;button.textContent=`Provar Stories del ${label}`;}
}
