...
assert.match(worker, /finalAttempts >= BUFFER_X_MAX_ATTEMPTS/);
...
assert.match(worker, /assets = \[\{ image:\{ url:await ensureSocialCardUrl\(socialDraft, env\) \} \}\]/);
assert.match(worker, /\(request\.method === 'GET' \|\| request\.method === 'HEAD'\) && socialCardMatch/);
assert.match(worker, /request\.method === 'HEAD' \? null : cached\.body/);
assert.match(worker, /remoteId=cleanText\(error\.remoteId,120\) \|\| null/);
assert.match(worker, /remoteId\?'remote_delivery_failed':'publish_failed'/);
