...
import {summarizeEnvironment,chooseEnvironmentScene,environmentalCaption,secondaryUv} from '../scripts/social-pilot/environment.mjs';
...
assert.equal(chooseEnvironmentScene(args).kind,'rain');
assert.match(environmentalCaption(summary,chooseEnvironmentScene(args),options),/no és una mesura/);
assert.equal(secondaryUv(summary,options).value,6);
assert.equal(secondaryUv(summary,{...options,date:'2026-09-21'}),null);
assert.equal(secondaryUv(summary,{...options,now:now+91*60_000}),null);
assert.equal(secondaryUv({...summary,uv:null},options),null);
assert.equal(environmentalCaption({...summary,uv:null},chooseEnvironmentScene(args),options),null);
const zero=structuredClone(payload);zero.hourly.uv_index.fill(0);
assert.equal(secondaryUv(summarizeEnvironment(zero,options),options).value,0);
...
assert.equal(chooseEnvironmentScene({...args,forecast:wet}).kind,'rain');
...
assert.equal(chooseEnvironmentScene({...args,summary:{...summary,air:{...summary.air,value:70}}}).kind,'rain');
assert.equal(chooseEnvironmentScene().kind,'rain');
const future=structuredClone(payload);future.hourly.time=future.hourly.time.map(t=>t.replace('2026-09-20','2026-09-21'));
const futureOptions={...options,date:'2026-09-21'};
assert.equal(secondaryUv(summarizeEnvironment(future,futureOptions),futureOptions).value,6);
