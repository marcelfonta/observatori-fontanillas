...
for(const period of ['mati','migdia','vespre'])for(const stress of [false,true]){
 test(`production card ${period} ${stress?'missing-long':'regular'}`,async({page},info)=>{
  ...
  const errors=await page.evaluate(()=>{
   ...
   for(const article of document.querySelectorAll('article'))if(rect(article.querySelector('.values')).bottom>rect(article).bottom-8)errors.push('Values too close to divider');
