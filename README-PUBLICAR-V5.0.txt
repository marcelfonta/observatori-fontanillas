OBSERVATORI FONTANILLAS · PUBLICAR LA VERSIÓ 5.0

1. Copia tot el contingut d’aquesta carpeta dins de la teva carpeta habitual:

/Users/marcelfontanillas/Library/Mobile Documents/com~apple~CloudDocs/observatori-fontanillas

Quan el Mac ho pregunti, accepta substituir els fitxers amb el mateix nom.

2. Obre Terminal directament dins de la carpeta observatori-fontanillas.

3. Executa aquestes tres ordres, una per una:

git add .
git commit -m "Publica la versió 5.0 del dashboard"
git push origin main

4. Cloudflare iniciarà el desplegament automàtic. Normalment el canvi apareix al web al cap d’un o dos minuts.

5. Si encara veus la versió anterior, recarrega meteo.fontanillas.cat amb Command + Shift + R.

No cal modificar el Worker ni les claus secretes per publicar aquesta versió. Meteocat, AEMET, eltiempo.es, Yr i Meteoblue s’integren mitjançant recursos externs fiables. El selector per hores o dies apareix només dins de Meteocat, perquè és l’únic d’aquests ginys que permet aquest canvi directament. Meteoblue mostra una previsió detallada cada 3 hores durant 4 dies.

La versió 5.0 també incorpora avisos meteorològics oficials: mapa vigent de Meteocat, escala de perill i accessos directes al Vallès Oriental i al Prelitoral de Barcelona d’AEMET.
