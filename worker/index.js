...
async function recentTemperatureTrend(env, referenceEpoch = null) {
  if (!env.DB || !(await ensureSchema(env))) return null;
  let endEpoch = optionalFinite(referenceEpoch);
  if (endEpoch === null) {
    const latest = await env.DB.prepare('SELECT MAX(observed_epoch) AS epoch FROM observations WHERE temperature IS NOT NULL').first();
    endEpoch = optionalFinite(latest?.epoch);
  }
  if (endEpoch === null) return null;
  const roundedEnd = Math.floor(endEpoch);
  const rows = await env.DB.prepare(`SELECT observed_epoch AS epoch, temperature
    FROM observations
    WHERE observed_epoch >= ? AND observed_epoch <= ?
      AND temperature BETWEEN -50 AND 60
    ORDER BY observed_epoch`)
    .bind(roundedEnd - 86400, roundedEnd).all();
  const trend = summarizeTemperatureTrend(rows?.results || []);
  return trend && trend.hours >= 6 ? trend : null;
}

async function temperatureTrend(env) {
  const trend = await recentTemperatureTrend(env);
  return json({ ok:Boolean(trend), windowHours:24, trend }, 200, 'public, max-age=300');
}
...
function socialSlotProfile(slot='06:45'){
...
const SOCIAL_SCHEDULE_BLUEPRINT=[
  {time:'06:45',period:'mati',label:'Bon dia i previsió',purpose:'Dades reals, previsió d’avui i avanç de demà'},
...
const YOUTUBE_SHORT_FALLBACK_WINDOWS={ mati:{ hour:6, minute:5 }, vespre:{ hour:19, minute:45 } };
...
export function officialAlertTiming(entry,date=new Date()){
  const today=localIsoDate(date);
  const validDate=officialAlertCardDate(entry||{});
  if(!validDate)return {targetDate:null,offset:null,isFuture:false,shortLabel:'una data pendent de confirmar',longLabel:'una data pendent de confirmar'};
...
// Older drafts have county periods but no targetDate. Never substitute today's
// date or the episode-wide start for an unknown county validity date.
export function officialAlertCardDate(data){
  const valid=value=>/^\d{4}-\d{2}-\d{2}$/.test(value||'') &&
    Number.isFinite(Date.parse(`${value}T12:00:00Z`)) &&
    new Date(`${value}T12:00:00Z`).toISOString().slice(0,10)===value;
  if(data.targetDate)return valid(data.targetDate)?data.targetDate:null;
  const periods=Array.isArray(data.periods)?data.periods:[];
  const first=periods[0]||String(data.description||'').match(/Franges:\s*([^.]*)/)?.[1]||'';
  const match=String(first).match(/^(\d{2})\/(\d{2})\s/);
  const anchor=[data.starts,data.expires].find(value=>valid(String(value||'').slice(0,10)));
  if(!match||!anchor)return null;
  const year=Number(String(anchor).slice(0,4));
  const candidates=[year-1,year,year+1].map(y=>`${y}-${match[2]}-${match[1]}`).filter(valid);
  candidates.sort((a,b)=>Math.abs(Date.parse(a)-Date.parse(anchor))-Math.abs(Date.parse(b)-Date.parse(anchor)));
  return candidates[0]||null;
}

function officialAlertCardTimingMarkup(data,draft){
  const targetDate=officialAlertCardDate(data);
  if(!targetDate)return '<section class="alert-validity"><small>VIGÈNCIA</small><strong>DATA PENDENT DE CONFIRMAR</strong><span>Consulta les franges i el detall oficial.</span></section>';
  const dateLabel=new Intl.DateTimeFormat('ca-ES',{timeZone:TIME_ZONE,weekday:'long',day:'numeric',month:'long'}).format(new Date(`${targetDate}T12:00:00Z`)).replace(',', '').toLocaleUpperCase('ca-ES');
  const issuedRaw=data.issuedAt||draft.created_at;
  const issued=issuedRaw?new Date(/Z$|[+-]\d{2}:\d{2}$/.test(issuedRaw)?issuedRaw:`${String(issuedRaw).replace(' ','T')}Z`):null;
  const issuedDay=issued&&Number.isFinite(issued.getTime())?localIsoDate(issued):null;
  const future=issuedDay&&targetDate>issuedDay;
  const stamp=issuedDay?`Publicació del ${issuedDay.split('-').reverse().join('/')} · hores locals`:'Hores locals';
  return `<section class="alert-validity"><small>PREVIST PER</small><strong>${escapeHtml(dateLabel)}</strong>${future?'<span class="alert-future">NO ÉS PER AVUI · AVÍS ANTICIPAT</span>':''}<small class="alert-issued">${escapeHtml(stamp)}</small></section>`;
}
...
export function officialAlertSocialCopy(entry,date=new Date()){
  ...
  const body=`${temporalNote}\n\n⚠️ Avís oficial ${levelLabel} de Meteocat per ${entry?.phenomenon||'fenomen meteorològic'} al ${area}. ${precision} ${cleanText(entry?.description||entry?.title,820)} Consulta sempre el detall oficial i segueix les indicacions de Protecció Civil.\n\n${socialHashtags('official_alert')}`;
...
function socialCardCacheKey(draftId, format = 'png', kind = '') {
  const id = Number(draftId);
  if (!Number.isSafeInteger(id) || id < 1) throw new Error('Identificador de targeta social no vàlid.');
  return `${SOCIAL_CARD_CACHE_PREFIX}${kind==='official_alert'?'validity-v2/':''}${id}.${format === 'jpeg' ? 'jpg' : 'png'}`;
}
...
async function invalidateSocialCardCache(draftId, env) {
  ...
  await env.SOCIAL_VIDEO_BUCKET.delete([
    ...
    socialCardCacheKey(draftId, 'png', 'official_alert'),
    socialCardCacheKey(draftId, 'jpeg', 'official_alert'),
  ]);
}
...
const BUFFER_TIKTOK_SLOT_TIMES = { morning:{ hour:6, minute:45 }, evening:{ hour:20, minute:30 } };
const BUFFER_TIKTOK_RECOVERY_STARTS = { morning:{ hour:6, minute:5 }, evening:{ hour:19, minute:45 } };
const BUFFER_TIKTOK_RECOVERY_AFTER_MINUTES = 90;
const BUFFER_X_SLOT_TIMES = { morning:{ hour:6, minute:45 }, midday:{ hour:14, minute:0 }, evening:{ hour:20, minute:30 } };
const BUFFER_X_RECOVERY_STARTS = { morning:{ hour:6, minute:5 }, midday:{ hour:14, minute:0 }, evening:{ hour:19, minute:45 } };
...
function dailyTemperatureTrendMarkup(trend) {
  const geometry = temperatureTrendGeometry(trend, { width:440, height:112, paddingX:5, paddingY:9 });
  if (!geometry) return '';
  const hours = optionalFinite(trend?.hours);
  const label = hours !== null && hours >= 20 ? 'ÚLTIMES 24 H · DADES OBSERVADES' : 'EVOLUCIÓ RECENT · DADES OBSERVADES';
  const change = optionalFinite(trend?.change);
  const changeLabel = change === null ? '' : `${change > 0 ? '↗ +' : change < 0 ? '↘ ' : '→ '}${reportMetric(change,'°',1)}`;
  return `<div class="temperature-trend"><div class="temperature-trend-head"><span>${label}</span>${changeLabel?`<b>${escapeHtml(changeLabel)}</b>`:''}</div><svg viewBox="0 0 440 112" preserveAspectRatio="none" role="img" aria-label="Evolució observada de la temperatura"><path class="temperature-trend-area" d="${geometry.area}"/><path class="temperature-trend-line" d="${geometry.path}"/><circle cx="${geometry.current.x}" cy="${geometry.current.y}" r="6"/></svg><div class="temperature-trend-extremes"><span>Mín. <b>${escapeHtml(reportMetric(trend.minimum,'°',1))}</b></span><span>Màx. <b>${escapeHtml(reportMetric(trend.maximum,'°',1))}</b></span></div></div>`;
}
...
async function enrichedSocialCardDraft(draft, env) {
  let data = {};
  try { data = JSON.parse(draft.payload || '{}'); } catch {}
  const missingMetrics = ['feelsLike','pressure','windGust'].some(key => finite(data[key]) === null);
  const missingTrend = draft.kind === 'daily_observation' && !data.temperatureTrend;
  if ((!missingMetrics && !missingTrend) || !env.DB) return draft;
  const observedAt = cleanText(data.observationUpdated, 32);
  const localDate = cleanText(data.localDate, 20);
  let row = null;
  if (observedAt) {
    row = await env.DB.prepare(`SELECT observed_epoch,local_time,temperature,feels_like,humidity,pressure,wind_speed,wind_gust,rain_total
  ...
  if (missingTrend) {
    const trend = await recentTemperatureTrend(env,row.observed_epoch).catch(error=>{console.error('Social card temperature trend error',error);return null;});
    if (trend) data.temperatureTrend = trend;
  }
...
async function materializeSocialCard(draft, env, format = 'png') {
  ...
  const key = socialCardCacheKey(draft.id, format, draft.kind);
...
async function socialCard(request, env, draftId, url, format = 'png') {
  ...
  if (env.SOCIAL_VIDEO_BUCKET) {
    const cached = await env.SOCIAL_VIDEO_BUCKET.get(socialCardCacheKey(draftId, format, storedDraft.kind));
    if (cached) {
      const headers = new Headers();
...
async function adminStatus(request, env) {
  ...
  return json({
    ...
    schedule:{ observationMinutes:STORAGE_INTERVAL_MINUTES, alerts:"cada 5 minuts", social:String(env.SOCIAL_AUTO_TIMES || DEFAULT_SOCIAL_AUTO_TIMES), preflight:String(env.SOCIAL_PREFLIGHT_TIME || DEFAULT_SOCIAL_PREFLIGHT_TIMES), youtube:'06:05,19:45', metaVideo:String(env.META_VIDEO_AUTO_TIMES || DEFAULT_META_VIDEO_AUTO_TIMES), metaVideoEnabled:metaVideoAutomationEnabled(env), timeZone:TIME_ZONE },
