...
import {summarizeEnvironment,chooseEnvironmentScene,environmentalCaption} from './environment.mjs';
...
const output=environment?resolve(root,evening?'v4-evening':'v4-environment'):lunar?resolve(root,'v3-lunar'):v2?resolve(root,'v2'):root;await mkdir(output,{recursive:true});
...
if(evening){
 ...
 e.caption=environmentalCaption(e.summary,e.selection);
}
...
if(environment&&!evening){
 ...
 await writeFile(resolve(output,'caption.txt'),data.environment.caption||'Mapa de precipitació: prioritat meteorològica o indicadors ambientals insuficients.');
}
...
if(environment){
 for(const kind of ['uv','air'])for(const value of [0,3,60,101,999]){
  ...
  const result=await page.evaluate(d=>window.renderPilot(28.2,d),fixture);
  if(result.errors.length)throw Error('Environmental layout: '+JSON.stringify(result.errors));stressCases++;
 }
 if(!evening){
...
const report={version:environment?4:lunar?3:v2?2:1,environment:data.environment?.selection,moon:data.moon,date:data.date,capturedAt:data.capturedAt,resolution:'1080x1920',duration:30,scenes:6,safeTextBounds:{left:76,right:946,top:185,bottom:1710},syntheticStressCases:stressCases,pageErrors:errors,publishing:false};
