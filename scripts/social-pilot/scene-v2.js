...
function forecast(i,t,local){
 ...
 if(i===2&&data.lunarEnabled){
  ...
  const uv=i===1&&data.environmentEnabled?data.environment.secondaryUv:null;
  if(uv){
   text(`UV màx. previst: ${num(uv.value,1)} · ${uv.peakTime.slice(11,16)} h`,88,790,27,C.muted,'DM Sans',550);
   text('Màxim horari del dia · CAMS via Open-Meteo',88,827,23,C.muted,'DM Sans',550);
  }else text('01 / 03'.replace('01','0'+(i+1)),88,810,25,C.muted,'DM Sans',210);
 }
 thermal(i,t,local);
...
window.renderMidday=()=>{
 ...
 text(`UV màx. previst: ${num(s.uv?.value,1)}`,72,1081,27,C.muted,'DM Sans',450);
...
function single(i,local,t){
 ...
 boxes=[];backdrop(t,i);if(i<3)forecast(i,t,local);else if(i===3)measured(t,local);else if(i===4)nextDays(t,local);else mapScene(t,local);identity(t,i);
